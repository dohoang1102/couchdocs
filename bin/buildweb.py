...
class Site(object):    
    ...
    def render(self):
        ...
        for curr_path, dirs, files in os.walk(INPUT_PATH):
            ...
            self.process(files, utf8(curr_path), utf8(tgt_path))
...
class Page(object):
    ...
    def __init__(self, site, filename, curr_path, tgt_path):
        ...
        newext = self.headers.get('ext', u'.html')
        self.target = os.path.join(tgt_path, u"%s%s" % (basename, newext))
    ...
    def convert_md(self, body):
        ...
        return markdown(body, ['wikilinks'])
