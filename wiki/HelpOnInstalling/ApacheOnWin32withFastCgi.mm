...
The Moin project recommends <<HelpOnInstalling/ApacheWithModWSGI|mod_wsgi>> as the preferred python accelerator. It is faster performing, and easier to set up & configure under Windows than FastCGI. For example, mod_wsgi does not require being run as a Windows Service, which eliminates a whole section of this page.

If you have a working FastCGI on Windows, and want to consider switching, I have <<http://moinmo.in/RickVanderveer/UpgradingFromFastCGItoModWSGI|switching instructions here>>. 
}}}
