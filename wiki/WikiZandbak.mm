...
Ga je gang, experimenteer hieronder na de vier koppeltekens... en alsjeblieft, maak '''GEEN''' nieuwe pagina's zonder betekenisvolle inhoud alleen maar om het uit te proberen!

'''Tip:''' Open "HulpBijBewerken" in een nieuw venster.
----

== Opmaak ==

''cursief'' '''vet''' {{{typemachine}}} 

`terugquote typemachine` (optie in de configuratie)
...
HulpBijBewerken MoinMoin:InterWiki 

[:HelpOnEditing:In het Engels]
...
=== Punten ===
 * eerste
   1. genest en genummerd
   1. genummerde lijsten hebben nummers
 * tweede
 * derde
   laten we eens zien
 blockquote
