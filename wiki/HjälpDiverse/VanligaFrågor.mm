...
##master-page:HelpMiscellaneous_2fFrequentlyAskedQuestions
##master-date:2002-03-15 10:00:52
...
Ordet ''Wiki'' är en förkortad form av ordet Wiki''''''Wiki''''''Webb.  En Wiki är en databas med sidor som en grupp människor kan redigera med en vanlig webbläsare.

=== Vad är ''MoinMoin''? ===

En Moin''''''Moin är en wiki som drivs av pythonwikiprogrammet vid namn Moin''''''Moin.

=== Vad är den bra för? ===

I ärlighetens namn är den bra för vad helst man använder den till. På
[http://www.lineo.com/ Lineo] finns en wiki på en intern webbplats med
väldigt många sidor som beskriver olika projekt, arkitekturer, idéer etc. 
för människor att kommentera på. En del sidor finns där bara för att ge information. Andra sidor är öppna inviter för diskussioner och kommentarer. Det finns också en wiki för en mycket enkel databas.
...
Wikier används internt av de som skriver [http://www.zope.org/ Zope] för att samordna deras idéer och projekt.
 ...
 * Klicka på <<GetText(WordIndex)>>. Detta ger en alfabetiskt sorterad lista på alla ord som finns i sidtitlar. (Exempel, en sida med namn Blå``Tand är listad både under B / Blå såväl som under T / Tand).
...
Klicka på länken <<GetText(LatestChanges)>> högst upp på sidan.
 ...
 * Redigera Wiki-sidan (gå till Wiki-sidan och klicka Redigera''''''Text-länken)
...
Om du använder MoinMoin på ett IntraNet kan det vara användbart att skapa länkar till dokument, som är tillgängliga överallt, på din wiki. Till exempel:
{{{[file://servernamn/full/path/till/filen/filnamn.typ Klicka här för att läsa filnamn.typ]}}}
Du kan också länka till filnamn med mellanrum i filnamnet genom att separat använda URL-koden för mellanrum ({{{%20}}}):
{{{
...
Om de är signifikanta eller om du vill att andra ska veta att du har gjort dem, så är svaret ja.
Skriv bara en epost-adress efter din kommentar. Det är inte ovanligt att göra ett indrag på din kommentar relativt det du kommenterar. Ibland hjälper det också att göra kommentaren i ''kursiv'' stil för att göra den mer synlig relativt det du kommenterar.
...
Om du vill lägga in en enstaka rad med HTML-kod, använd HTML-makrot. Detta gör du genom att skriva din HTML-text som en parameter till HTML-makrot. Exempel:
{{{<<HTML(<font size=+12>Detta är ett stort typsnitt</font>)>>}}}

Detta visar sig som
<<HTML(<font size=+12>Detta är ett stort typsnitt</font>)>>

Det är också möjligt att ha en hel sida i HTML-format genom att sätta
{{{#format html}}}
som allra första rad i sidan. Om den raden är där så kommer hela sidan att tolkas som HTML (vilket samtidigt gör länkar till andra sidor lite svåra att tyda!)
Om du gör det, försäkra dig om att du bara lägger in huvuddelen av HTML-sidan och inte HTML-huvuden eller något annat utanför detta, inkluderat <BODY>-taggen själv.
 /!\ Detta fungerar endast om HTML-funktionen (HTML makro och parser) är installerad på wikin.

== Installation och anpassning ==
 (!) se HjälpMedAdministration för generell hjälp om detta

=== Varför fungerar inte "diff"-funktionen? ===
Diffar behöver två saker för att fungerar korrekt:
 * Filkatalogen "''datadir''{{{/backup}}}" måste existera i installationen.
 * Programmet GNU diff måste finnas på systemet och i web-serverns `PATH`-variabel. Observera att `PATH:en` som webb-servern använder kan vara kortare än den du får i "shell":en
...
Det finns en mängd administrationsinformation på <<GetText(LatestChanges)>>-sidan, inklusive antalet sidor och de olika makros och åtgärder som är installerade.

Jag sätter vanligtvis upp en "Administratörs''''''Sida", där jag placerar olika makros för administration, jämte information om var sidorna är lokaliserade, samt annat som en administration bör känna till eller har användning för.
