...
Oben rechts sieht man einige Icons für die am häufigsten benutzten Funktionen:
  * <<Icon(up)>> springt zu der übergeordneten Seite und wird nur dann angezeigt, wenn Sie sich auf einer Unterseite befinden (siehe HilfeZumEditieren/UnterSeiten)
  * <<Icon(view)>> aktuelle Seite neu laden
  * <<Icon(print)>> Anzeigen einer Druckversion der Seite ohne Kopf- und Fußbereich
  * <<Icon(raw)>> zeigt den Quellcode der aktuellen Seite an
  * <<Icon(subscribe)>>/<<Icon(unsubscribe)>> ermöglicht es, Email-Benachrichtigungen für die aktuelle Seite anzuschalten, die hellere Version bedeutet, dass man bereits Benachrichtigungen erhält
  * <<Icon(edit)>> erlaubt es, die Seite zu editieren ("etwas zu sagen")
  * <<Icon(info)>> zeigt Metadaten über die Seite an, insbesondere die gesamte Versions-Historie
  * <<Icon(diff)>> zeigt Änderungen zwischen der aktuellen und der vorherigen Version der Seite an
  * <<Icon(find)>> zeigt die Suchseite an
  * <<Icon(help)>> zeigt die Hilfeseite
